(function() {
	...
	function getWorkflowParams(eventid, triggerid) {
		...
		if (triggerid) {
			params.triggerid = String(triggerid);
		}
		return params;
	}

	function createWorkflowIcon(eventid, triggerid) {
	...
	function injectIconsIntoTable(table) {
		...
		rows.forEach(function(row) {
			if (row.querySelector('.mnz-problem-workflow-icon')) return;
			...
			const investigationIcon = insertCell.querySelector('.mnz-problem-investigation-icon');
			const icon = createWorkflowIcon(ids.eventid, ids.triggerid);
			const spacer = document.createTextNode('\u00A0');

			if (investigationIcon && investigationIcon.nextSibling) {
				investigationIcon.parentNode.insertBefore(spacer, investigationIcon.nextSibling);
				investigationIcon.parentNode.insertBefore(icon, investigationIcon.nextSibling);
			} else {
